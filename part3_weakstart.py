...
"""
part3_weakstart.py
Multi-round Part 3 codebook revision starting from a WEAK bare-bones codebook.
Scores with Cohen's kappa (pooled + per-code). Conservative reviser edits.

Condition: LABEL | Dataset: TUTORING | N_ROUNDS = 5
Holdout: reviser sees model-agreed anchors + label-disagreement cases; never human gold.
"""
...
OUT = BASE / "part3_weakstart_results"
...
WEAK_CODEBOOK: dict[str, str] = {
    "Greeting": "greeting or acknowledging the student.",
    "Instruction": "telling the student what to do or explaining how to do it.",
    "Guiding feedback": (
        "guiding the student toward the answer without giving it directly."
    ),
    "Aligning to prior knowledge": (
        "connecting to something the student learned before."
    ),
    "Understanding/Engagement-Tutor": (
        "checking whether the student understands, usually by asking a question."
    ),
    "Encouragement": "praising or motivating the student.",
}
...
def pooled_conf_counts(records: list[dict], pred: dict[str, dict]) -> dict[str, int]:
    tp = fp = fn = tn = 0
    for r in records:
        iid = r["item_id"]
...
def accuracy_report(records: list[dict], pred: dict[str, dict]) -> tuple[float, dict]:
    ...
    for code in TUT_CODES:
        correct = sum(
            1
            for r in records
            if pred[r["item_id"]].get(code, 0) == r["labels"].get(code, 0)
        )
...
def kappa_report(
    records: list[dict], pred: dict[str, dict]
) -> tuple[float, dict[str, float]]:
    pooled = pooled_conf_counts(records, pred)
    pooled_k = cohen_kappa(pooled["TP"], pooled["FP"], pooled["FN"], pooled["TN"])
    per_code: dict[str, float] = {}
    for code in TUT_CODES:
        c = conf_counts(records, pred, code)
        per_code[code] = cohen_kappa(c["TP"], c["FP"], c["FN"], c["TN"])
    return pooled_k, per_code
...
- Preserve each code's core meaning (e.g. Understanding/Engagement-Tutor = tutor checks
  understanding, usually by asking a question).
...
def record_metrics(
    ...
    pooled_k, per_k = kappa_report(records, pred)
    overall_acc, _ = accuracy_report(records, pred)
    ue = conf_counts(records, pred, FOCUS)
    row = {
        "round": round_num,
        "pooled_kappa": f"{pooled_k:.4f}",
    ...
    print(
        f"    Round {round_num}: pooled_kappa={pooled_k:.4f}  acc={overall_acc:.4f}  "
        f"UE_kappa={per_k[FOCUS]:.4f}  UE TP/FP/FN/TN="
        f"{ue['TP']}/{ue['FP']}/{ue['FN']}/{ue['TN']}  "
...
def trend_read(rows: list[dict]) -> str:
    ...
    if delta >= 0.02:
        return (
            f"pooled kappa RISES over rounds ({start:.4f} -> {final:.4f}, "
            f"delta={delta:+.4f}) — revision improving the weak codebook"
        )
    if delta <= -0.02:
        return (
            f"pooled kappa FALLS ({start:.4f} -> {final:.4f}, delta={delta:+.4f}) — "
            "revision did not help or hurt agreement with human gold"
        )
    monotonic_up = all(kappas[i] <= kappas[i + 1] + 0.005 for i in range(len(kappas) - 1))
    if monotonic_up and delta > 0:
        return (
            f"pooled kappa rises steadily ({start:.4f} -> {final:.4f}) — "
            "revision improving the weak codebook"
        )
    return (
        f"pooled kappa mixed/plateau ({start:.4f} -> {final:.4f}, delta={delta:+.4f})"
    )
...
def main() -> None:
    ...
    results_csv = OUT / "part3_weakstart_results.csv"
    summary_path = OUT / "part3_weakstart_summary.txt"

    print("=" * 70)
    print("PART 3 WEAK-START — LABEL | TUTORING | kappa metric | conservative reviser")
    print(f"Rounds: 0 (weak baseline) + {N_ROUNDS} revision rounds")
    ...
    summary_lines: list[str] = [
        "Part 3 Weak-Start — LABEL | TUTORING",
        f"Reviser: {REVISER_MODEL} (conservative) | Coders: {MODEL_A} + {MODEL_B}",
        "Metric: Cohen's kappa (pooled + per-code); accuracy secondary",
        f"Starting codebook: bare-bones weak definitions ({word_count(WEAK_CODEBOOK)} words)",
        f"Rounds: 0 baseline + {N_ROUNDS} revisions",
        "",
    ]

    current_cb = dict(WEAK_CODEBOOK)

    # ── Round 0: weak baseline ────────────────────────────────────────────────
    print("\n" + "=" * 70)
    print("ROUND 0 — WEAK BASELINE (bare-bones codebook, agent A)")
    print("=" * 70)

    round0_cb_path = OUT / "revised_codebook_round0.json"
    round0_cb_path.write_text(
        json.dumps({"codes": WEAK_CODEBOOK, "changes": []}, indent=2),
        encoding="utf-8",
    )
    print(f"  Saved {round0_cb_path.name}")

    scored0_path = OUT / "scored_round0_baseline.csv"
    pred0 = run_agent_a_scoring(tutor_rows, WEAK_CODEBOOK, scored0_path)

    row0 = record_metrics(0, tutor_rows, pred0, WEAK_CODEBOOK, 0, results_csv)
    ...
    for row in all_rows:
        line = (
            ...
            f"{row['codebook_word_count']:>6} {row['n_changes']:>4}"
        )
        print(line)
        summary_lines.append(line)
        summary_lines.append(
            f"         UE FP/FN/TN: {row['UE_FP']}/{row['UE_FN']}/{row['UE_TN']}"
        )
    ...
    zip_path = BASE / "part3_weakstart_results.zip"
